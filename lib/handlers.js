      ...
      if (tokenIsValid) {
        ...
        _data.read('users', phone, (err, userdata) => {
          if (!err && userdata) {
            ...
            delete userdata.hashedPassword // eslint-disable-line
              ...
              if (!deleteErr) {
                ...
                if (checksToDelete > 0) {
                  ...
                  userChecks.forEach((checkId) => {
                    // Delete the check
                    _data.delete('checks', checkId, (checkDeleteErr) => {
                      if (checkDeleteErr) {
                        deletionErrors = true
                      }
...
handlers._tokens.put = (data, callback) => {
  ...
  const id =
    typeof data.payload.id === 'string' && data.payload.id.trim().length === 20
      ...
      : false
  const extend =
    typeof data.payload.extend === 'boolean' && data.payload.extend === true // eslint-disable-line no-unneeded-ternary
      ? true
      : false
    ...
    _data.read('tokens', id, (err, tokenData) => {
      if (!err && tokenData) {
        ...
        if (tokenData.expires > Date.now()) {
          ...
          const expTokenData = tokenData
          expTokenData.expires = Date.now() + 1000 * 60 * 60

          // Store the new updates
          _data.update('tokens', id, expTokenData, (errUpdate) => {
            if (!errUpdate) {
              callback(200)
            } else {
              callback(500, {
  ...
  if (protocol && url && method && successCodes && timeoutSeconds) {
    ...
    _data.read('tokens', token, (err, tokenData) => {
      if (!err && tokenData) {
        ...
        _data.read('users', userPhone, (errUser, userData) => {
          if (!errUser && userData) {
            const userChecks =
              typeof userData.checks === 'object' &&
                userData.checks instanceof Array
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              // Save the object
              _data.create('checks', checkId, checkObject, (errCreate) => {
                if (!errCreate) {
                  // Add the check id to the user's object

                  userData.checks = userChecks // eslint-disable-line
                  userData.checks.push(checkId)

                  // Save the new user data
                  _data.update('users', userPhone, userData, (errUpdate) => {
                    if (!errUpdate) {
...
handlers._checks.put = (data, callback) => {
  ...
  if (id) {
    ...
    if (protocol || url || method || successCodes || timeoutSeconds) {
      // Lookup the check
      _data.read('checks', id, (err, checkData) => {
        const checkDataParam = checkData

        if (!err && checkDataParam) {
          // Get the token from the headers
          const token =
            typeof data.headers.token === 'string' ? data.headers.token : false
          ...
          handlers._tokens.verifyToken(
            token,
            checkDataParam.userPhone,
            (tokenIsValid) => {
              if (tokenIsValid) {
                // Update the check where necessary
                if (protocol) {
                  checkDataParam.protocol = protocol
                }
                if (url) {
                  checkDataParam.url = url
                }
                if (method) {
                  checkDataParam.method = method
                }
                if (successCodes) {
                  checkDataParam.successCodes = successCodes
                }
                if (timeoutSeconds) {
                  checkDataParam.timeoutSeconds = timeoutSeconds
                }

                // Store the new updates
                _data.update('checks', id, checkDataParam, (updateErr) => {
                  if (!updateErr) {
                    callback(200)
                  } else {
      ...
      callback(400, { Error: 'Missing field options to update.' })
    }
  } else {
    callback(400, { Error: 'Missing required fields.' })
  }
...
handlers._checks.delete = (data, callback) => {
  ...
  const id =
    typeof data.queryStringObject.id === 'string' &&
      data.queryStringObject.id.trim().length === 20 // eslint-disable-line
      ? data.queryStringObject.id.trim()
      : false
        ...
        handlers._tokens.verifyToken(
          ...
          (tokenIsValid) => {
            if (tokenIsValid) {
              ...
              _data.delete('checks', id, (deleteCheckErr) => {
                if (!deleteCheckErr) {
                  // Lookup the user
                  _data.read(
                    'users',
                    ...
                    (readErr, userData) => {
                      if (!readErr && userData) {
                        const userChecks =
                          typeof userData.checks === 'object' &&
                            userData.checks instanceof Array // eslint-disable-line
                            ? userData.checks
                            : []
